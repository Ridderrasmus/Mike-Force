...
PARADIGM_GITHUB_ZIP_URL = (
    "https://github.com/ScarletPigs/Paradigm/archive/refs/heads/development.zip"
)
...
def maybe_tear_down_builddir() -> None:
    ...
    if ARGS.build_dirpath.exists():
        ...
        LOGGER.debug(
            f"Removed temporary build directory: path={ARGS.build_dirpath}")
...
def create_build_subdir(dirname: str) -> Path:
    """
    Create a new subdirectory in the build path

    @param: dirname: name of the new build sub directory
    @return: pathlib.Path object referencing the location of the new sub directory
    """
    ...
    with LoggerManager("Building release archives.") as l:
        ...
        for archive_type in ["zip", "gztar"]:
            ...
            archive_name = shutil.make_archive(
                str(build_archivedir.joinpath(main_fname)),
                archive_type,
                build_stagedir,
            )
            l.info(
                f"Built archive file: type={archive_type} fname={main_fname}")
            ...
            for mission in build_stagedir.glob("*"):
                shutil.make_archive(
                    str(build_archivedir.joinpath(mission.name)),
                    archive_type,
                    build_stagedir,
                    mission.name,
                )
                l.info(
                    f"Built archive file: type={archive_type} fname={mission.name}")
